...
import { isGeneratedCodePath, isIgnoredDir, resolveInRoot, toRelPath } from "./paths.js";
...
const GENERATED_CONTENT_BYTES = 1200;
const IMPORTED_BY_MAX = 80;
const EXCLUDED_ROOTS_MAX = 8;

export interface CodeMapExclusionConfig {
  exclude: string[];
  include: string[];
}
...
function matchesGlob(rel: string, pattern: string): boolean {
  ...
  return globRegExp(normalized).test(normalized.includes("/") ? rel : path.posix.basename(rel));
}

function generatedPathRoot(rel: string): string | null {
  const parts = rel.split("/");
  const generatedAt = parts.findIndex((part) => part === "generated" || part === "__generated__");
  if (generatedAt >= 0) return parts.slice(0, generatedAt + 1).join("/");
  const name = parts.at(-1) ?? "";
  if (name.includes(".generated.") || name.endsWith(".pb.ts") || name.endsWith("_pb.ts")) {
    return path.posix.dirname(rel) === "." ? "." : path.posix.dirname(rel);
  }
  return null;
}

function configExcludeRoot(rel: string, patterns: readonly string[]): string | null {
  const match = patterns.find((pattern) => matchesGlob(rel, pattern));
  if (!match) return null;
  const normalized = match.replace(/\\/g, "/").replace(/^\.\//, "");
  const wildcard = normalized.search(/\*/);
  const fixed = (wildcard < 0 ? normalized : normalized.slice(0, wildcard)).replace(/\/$/, "");
  if (!fixed || !normalized.includes("/")) return path.posix.dirname(rel) || ".";
  return wildcard < 0 ? path.posix.dirname(fixed) || "." : fixed;
}

export function hasGeneratedMarker(text: string): boolean {
  return /@generated|do not edit|code generated by|this file was automatically generated/i.test(
    text.slice(0, GENERATED_CONTENT_BYTES),
  );
}

export function createCodeMapExclusionMatcher(
  config: CodeMapExclusionConfig,
): (rel: string) => boolean {
  return (rel: string): boolean => {
    const normalized = rel.replace(/\\/g, "/").replace(/^\.\//, "");
    if (config.include.some((glob) => matchesGlob(normalized, glob))) return false;
    return (
      isGeneratedCodePath(normalized) ||
...
export class CodeMapAnalyzer {
  ...
  private exclusionConfig: CodeMapExclusionConfig = { exclude: [], include: [] };
  private excluded: CodeMapSummary["excluded"];
  private overviewMemo: {
    generation: number;
  ...
  invalidate(): void {
    this.dirty = true;
    this.overviewMemo = null;
  }

  /** Cheap watcher predicate using the config loaded by the latest analysis. */
  isExcludedPath(rel: string): boolean {
    return createCodeMapExclusionMatcher(this.exclusionConfig)(rel);
  }

  private async loadExclusionConfig(): Promise<void> {
    try {
      const value: unknown = JSON.parse(
        await fs.readFile(resolveInRoot(this.root, ".crystal/codemap.json"), "utf8"),
      );
      ...
      this.exclusionConfig = { exclude: [], include: [] };
    }
  }
  ...
  private async analyze(): Promise<void> {
    ...
    await this.loadExclusionConfig();
    const moduleDirs = await this.discoverModules();
    let files = await this.collectFiles(moduleDirs);
    const excludedRoots = new Map<string, number>();
    const exclude = (rel: string, root: string): void => {
      excludedRoots.set(root, (excludedRoots.get(root) ?? 0) + 1);
      this.parseCache.delete(rel);
    };
    files = files.filter((file) => {
      if (!this.isExcludedPath(file.rel)) return true;
      exclude(
        file.rel,
        generatedPathRoot(file.rel) ??
          configExcludeRoot(file.rel, this.exclusionConfig.exclude) ??
          path.posix.dirname(file.rel),
      );
      return false;
    });
        ...
        try {
          ...
          if (!parsed || parsed.mtimeMs !== stat.mtimeMs) {
            try {
              ...
              if (
                !this.exclusionConfig.include.some((glob) => matchesGlob(rel, glob)) &&
                hasGeneratedMarker(text)
              ) {
                exclude(rel, path.posix.dirname(rel) === "." ? "." : path.posix.dirname(rel));
                return;
              }
    ...
    this.excluded =
      excludedRoots.size > 0
        ? {
            files: [...excludedRoots.values()].reduce((sum, count) => sum + count, 0),
            roots: [...excludedRoots.entries()]
              .map(([path, files]) => ({ path, files }))
              .sort((a, b) => b.files - a.files || a.path.localeCompare(b.path))
              .slice(0, EXCLUDED_ROOTS_MAX),
          }
        : undefined;
    this.modules = moduleDirs
      .map((m) => ({
        ...m,
        fileCount: [...records.values()].filter((r) => r.module === m.path).length,
      }))
      // In a synthesized single-package map, "." remains the graph's package
      // anchor even when all source files belong to derived child modules.
      .filter(
        (m) => m.fileCount > 0 || (m.path === "." && moduleDirs.some((item) => item.synthetic)),
      );
  ...
  async summary(): Promise<CodeMapSummary> {
    ...
    return {
      ...
      ...(this.excluded ? { excluded: this.excluded } : {}),
  ...
  async fileDetail(rel: string): Promise<CodeFileDetail> {
    ...
    const importedBy = [...(this.importedBy.get(rel) ?? [])].sort();
    return {
      path: rel,
      module: record.module,
      loc: record.loc,
      ...
      importedBy: importedBy.slice(0, IMPORTED_BY_MAX),
      ...(importedBy.length > IMPORTED_BY_MAX ? { importedByTotal: importedBy.length } : {}),
    };
  }
  ...
  /** Logical overview cached for the current completed analyzer generation. */
  async systemOverview(index: CodeIndex | null): Promise<SystemOverview> {
    await this.ensureFresh(true);
    const indexGeneratedAt = index?.generatedAt ?? null;
    if (
      this.overviewMemo?.generation === this.generation &&
      this.overviewMemo.indexGeneratedAt === indexGeneratedAt
    ) {
      return this.overviewMemo.value;
    }

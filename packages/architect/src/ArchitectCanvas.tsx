...
import { isContainerKind, type ArchNodeKind, type ArchitectureGraph } from "@crystal/core";
import {
...
function CanvasInner({ graph, onChange }: ArchitectCanvasProps) {
  ...
  const rfNodes = useMemo(() => toRfNodes(graph, selectedNodes), [graph, selectedNodes]);
  const rfEdges = useMemo(() => toRfEdges(graph, selectedEdges), [graph, selectedEdges]);

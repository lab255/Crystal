...
export function ComposeSuggestions(props: {
  ...
  if (!result || result.files.length === 0 || suggestions.length === 0) return null;
  ...
  return <div className="w-80 overflow-hidden rounded-lg border border-edge bg-surface-1 shadow-lg">
    <button type="button" className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-medium text-ink" onClick={() => setCollapsed((value) => !value)}>
      {collapsed ? <ChevronRight size={14} /> : <ChevronDown size={14} />}<Container size={14} />
      <span className="flex-1">Compose topology</span><span className="text-ink-muted">{suggestions.length}</span>
    </button>
    {!collapsed && <div className="border-t border-edge px-3 py-2">
      <div className="mb-2 flex items-center justify-between text-[11px] text-ink-muted"><span>{result.files.length} file{result.files.length === 1 ? "" : "s"}</span><button type="button" disabled={!envId || pending.length === 0} className="rounded border border-accent-blue/25 bg-accent-blue/12 px-2 py-1 text-accent-blue disabled:opacity-40" onClick={() => adopt(pending)}>Adopt all</button></div>
